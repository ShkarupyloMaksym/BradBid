...
\begin{itemize}
    ...
    \item Використання \texttt{MessageGroupId} = торгова пара (напр. "BTC\_USD") для гарантії порядку.
...
\begin{enumerate}
    ...
    \item Варіант 1 (Простий): Matcher Lambda пише JSON безпосередньо у Firehose $\rightarrow$ S3.
    \item Варіант 2 (Дешевий): EventBridge Pipe читає \texttt{trades.fifo} $\rightarrow$ записує в S3 (або через Lambda).
...
\begin{itemize}
    \item Локальні unit-тести для логіки мaтчингу (без AWS).
